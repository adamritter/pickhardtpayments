...
fn sum(q : &VecDeque<Vindex>, dis : &[i64]) -> i64 {
     let mut s:i64=0;
    for i in q {
        s+=dis[i.as_usize()];
    }
...
fn main(){
    ...
    let begin = Instant::now();
    let file=File::open("lightning.data").unwrap();
    let br=BufReader::new(file);
    let lines=br.lines();
    let num6=Regex::new(r"(\d+)\W+(\d+)\W+(\d+)\W+(\d+)\W+(\d+)\W+(\d+)").unwrap();
    ...
    for i in 0..lightning_data.len() {
        writeln!(&mut w, "{}", lightning_data[i].flow).unwrap();
    }
